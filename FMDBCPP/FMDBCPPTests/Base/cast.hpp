...
template <> struct __encode__<Data>
{ static const char *type() { return "#Data"; } }; __ENCODE_CV__(Data,);
__ENCODE_TYPE_POINTER(Data, ^#Data,);
...
template <> struct __encode__<String>
{ static const char *type() { return "#String"; } }; __ENCODE_CV__(String,);
__ENCODE_TYPE_POINTER(String, ^#String,);
...
const char *encode()
{
    if (std::is_base_of<CHTagBuf, std::remove_pointer<T>>::value) {
...
{
    return encode<std::remove_cv<T>>();
}

template <>
inline const char *encode<const char *>(const char *s)
{
    if (is_steady_pointer(s)) {
        return "*";
    }
    return "^c";
}
